...
export const addScreenToProject = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!req.file) {
      return res.status(400).json({ message: 'Image file from Figma is required.' });
    }

    // Upload image to Cloudinary and delete the temporary file.
    const uploadResult = await cloudinary.uploader.upload(req.file.path, { folder: "visual-brd-projects" });
...
export const addAnnotationToScreen = async (req: AuthRequest, res: Response) => {
    try {
        ...
        //@ts-ignore
        if (!project || project.owner.toString() !== req.user!._id.toString()) {
            return res.status(401).json({ message: 'Not Authorized to annotate this screen.' });
        }
...
export const updateAnnotationPosition = async (req: Request, res: Response) => {
  try {
    ...
    await AnnotationModel.findByIdAndUpdate(annotationId, {
      $set: { x: x, y: y },
    });
    res.status(204).send();
...
export const exportScreenAsExcel = async (req: AuthRequest, res: Response) => {
    try {
        ...
        if (!project || project.owner.toString() !== req.user!._id.toString()) {
            return res.status(401).json({ message: 'Not Authorized to export this screen.' });
        }

        if (!req.file) {
            return res.status(400).json({ message: "Annotated image file is required." });
        }
        ...
        res.setHeader('Content-Disposition', `attachment; filename="BRD-${screen.name}.xlsx"`);
